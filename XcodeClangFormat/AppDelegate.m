...
- (void)applicationWillFinishLaunching:(NSNotification*)notification {
  defaults = [[NSUserDefaults alloc] initWithSuiteName:@"XcodeClangFormat2"];
  ...
  if (bookmark) {
    ...
    if (url) {
      // Regenerate the bookmark, so that the
      // extension can read a valid bookmark
      ...
      // Remove the bookmark value from the
      // storage
